...
async function login(emailValue, motDePasseValue) {
  try {
    ...
    if (response.ok) {
      console.log(data);
      const connexionToken = data.token;
      localStorage.setItem("connexionToken", connexionToken); // Stocker le token
...
form.addEventListener("submit", (event) => {
  ...
  login(email, password); // Appelle la fonction de connexion
});
